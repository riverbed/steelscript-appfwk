...
class GenericReportView(views.APIView):
    ...
    def render_html(self, report, request, namespace, report_slug, isprint):
        ...
        missing_devices = set()
        for _id, fields in report.collect_fields_by_section().items():
            for _name, field_obj in fields.items():
        ...
        [all_fields.update(c) for c in list(fields_by_section.values())]
        ...
        for s in Section.objects.filter(report=report).order_by('position',
                                                                'title'):
            show = False
            for v in list(fields_by_section[s.id].values()):
...
class ReportAutoView(GenericReportView):
    ...
    def get(self, request, namespace=None, report_slug=None, widget_slug=None):
        ...
        for w in widgets:
            ...
            keys = list(form._tablefields.keys())
            criteria = dict(list(zip(keys, [None]*len(keys))))
...
class ReportPrintView(GenericReportView):
    ...
    def get_media_params(self, request):
        ...
        criteria = json.dumps(dict(list(zip(list(request.POST.keys()),
...
def create_report_history(request, report, widgets):
    ...
    for widget in widgets:
        ...
        if form.is_valid():
            ...
            logger.debug("TABLE OPTIONS: {}".format(widget_table.options))
...
class WidgetTokenView(views.APIView):
    ...
    def post(self, request, namespace=None,
        ...
        all_fields = {}
        fields_by_section = report.collect_fields_by_section()
        [all_fields.update(fields) for fields in list(fields_by_section.values())]
    ...
    def get(self, request, namespace, report_slug, widget_slug, job_id,
        ...
        if not job.done():
            ...
            try:
                ...
                if widget.rows > 0:
                    tabledata = list(job.values())[:widget.rows]
                else:
                    tabledata = list(job.values())
